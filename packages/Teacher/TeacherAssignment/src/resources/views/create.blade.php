...
@section('title', __('teacher-assignment::app.assignment.create'))
...
<div class="flex min-h-screen flex-col bg-slate-50">
    ...
    <header class="sticky top-0 z-10 flex items-center gap-4 border-b border-slate-200 bg-white/90 px-6 py-4 backdrop-blur">
        ...
        <div>
            ...
            <h1 class="mt-0.5 text-lg font-black text-slate-950">@lang('teacher-assignment::app.assignment.create')</h1>
        </div>
    </header>

    <div class="mx-auto w-full max-w-7xl px-6 py-4">
        <form action="{{ route('teacher.assignments.store') }}" method="POST" enctype="multipart/form-data" class="grid grid-cols-1 gap-5 lg:grid-cols-12">
            @csrf
            ...
            <section class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm space-y-4 lg:col-span-7">
                ...
                <div class="space-y-1">
                    ...
                    <textarea name="description" rows="5"
                              class="block w-full rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-sm font-bold text-slate-700 outline-none focus:border-green-300 focus:ring-2 focus:ring-green-50"
                              placeholder="{{ __('teacher-assignment::app.assignment.description_placeholder') }}">{{ old('description') }}</textarea>
                    <p class="text-xs font-semibold text-slate-400">@lang('teacher-assignment::app.assignment.desc_help')</p>
                </div>

                <div class="space-y-1">
                    <label class="text-sm font-bold text-slate-700">@lang('teacher-assignment::app.assignment.field_file')</label>
                    ...
                    <p class="text-xs font-semibold text-slate-400">@lang('teacher-assignment::app.assignment.file_help')</p>
                ...
                <div class="space-y-4">
                    <div class="grid gap-4 md:grid-cols-2">
                        ...
                        <div class="space-y-1">
                            ...
                            <input type="number" name="max_score" value="{{ old('max_score', 10) }}"
                                   class="block w-full rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-sm font-bold text-slate-700 outline-none focus:border-green-300 focus:ring-2 focus:ring-green-50"
                                   min="1" max="10" step="1">
                        ...
                        <div class="flex items-center gap-4 mt-2">
                            <label class="flex items-center gap-2 cursor-pointer">
                                <input type="checkbox" name="allow_late" id="allowLate" value="1"
                                       ...
                                       onchange="document.getElementById('lateDaysWrap').classList.toggle('hidden', !this.checked)">
                                <span class="text-sm font-bold text-slate-600">@lang('teacher-assignment::app.assignment.allow_late')</span>
                            </label>

                            <div id="lateDaysWrap" class="flex items-center gap-2 {{ old('allow_late') ? '' : 'hidden' }}">
                                <input type="number" name="late_days" value="{{ old('late_days', 3) }}"
                ...
                <div class="grid gap-6 md:grid-cols-3">
                    <div class="md:col-span-2 space-y-2">
                        ...
                        <div class="grid grid-cols-3 gap-3 mt-1">
                            <label class="submission-type-card flex flex-col items-center justify-center rounded-xl border-2 p-4 text-center cursor-pointer transition hover:border-green-500 hover:bg-green-50/50 {{ old('submission_type', 'both') === 'file' ? 'border-green-600 bg-green-50' : 'border-slate-200 bg-white' }}" data-value="file">
                                <input type="radio" name="submission_type" value="file" class="hidden" {{ old('submission_type', 'both') === 'file' ? 'checked' : '' }}>
                            ...
                            <label class="submission-type-card flex flex-col items-center justify-center rounded-xl border-2 p-4 text-center cursor-pointer transition hover:border-green-500 hover:bg-green-50/50 {{ old('submission_type', 'both') === 'text' ? 'border-green-600 bg-green-50' : 'border-slate-200 bg-white' }}" data-value="text">
                                <input type="radio" name="submission_type" value="text" class="hidden" {{ old('submission_type', 'both') === 'text' ? 'checked' : '' }}>
                            ...
                            <label class="submission-type-card flex flex-col items-center justify-center rounded-xl border-2 p-4 text-center cursor-pointer transition hover:border-green-500 hover:bg-green-50/50 {{ old('submission_type', 'both') === 'both' ? 'border-green-600 bg-green-50' : 'border-slate-200 bg-white' }}" data-value="both">
                                <input type="radio" name="submission_type" value="both" class="hidden" {{ old('submission_type', 'both') === 'both' ? 'checked' : '' }}>
                    ...
                    <div class="space-y-1">
                        ...
                        <select name="status"
                                class="block w-full h-11 rounded-xl border border-slate-200 bg-white px-4 text-sm font-bold text-slate-700 outline-none focus:border-green-300">
                            <option value="draft"     {{ old('status') === 'draft'     ? 'selected' : '' }}>@lang('teacher-assignment::app.assignment.status_draft')</option>
                            <option value="published" {{ old('status') === 'published' ? 'selected' : '' }}>@lang('teacher-assignment::app.assignment.status_published')</option>
                        </select>
                    </div>
                </div>
            </section>
...
document.querySelectorAll('.submission-type-card').forEach(card => {
    card.addEventListener('click', function(e) {
        document.querySelectorAll('.submission-type-card').forEach(el => {
