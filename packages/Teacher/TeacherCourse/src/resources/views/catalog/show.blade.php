            ...
            <div class="min-w-0 space-y-5">
                <section class="overflow-hidden rounded-xl border border-slate-200 bg-white">
                    <div class="grid md:grid-cols-[280px_minmax(0,1fr)]">
                        <div class="aspect-video bg-slate-950 md:aspect-auto">
                        ...
                        <div class="p-5 sm:p-6">
                            ...
                            <dl class="mt-5 grid grid-cols-2 gap-3 border-t border-slate-100 pt-4 text-center sm:grid-cols-4">
                                ...
                                <div><dt class="text-[10px] font-black uppercase text-slate-400">@lang('teacher-course::catalog.students')</dt><dd class="mt-1 font-black text-green-700">{{ number_format((int) $course->enrollment_count) }}</dd></div>
                            </dl>
                        </div>
                    </div>
                </section>
            ...
            <aside class="space-y-5 xl:sticky xl:top-5 xl:self-start">
                ...
                <section class="rounded-xl border border-slate-200 bg-white p-5">
                    ...
                    @if($canShowTeacherProfile)
                        <span class="mt-3 inline-flex rounded-full bg-green-50 px-2.5 py-1 text-[10px] font-black uppercase text-green-700">@lang('teacher-course::reviews.verified')</span>
                    @endif
                    @if($canShowTeacherProfile && $course->teacher->teacherProfile->biography)<p class="mt-3 text-sm font-semibold leading-5 text-slate-500">{{ $course->teacher->teacherProfile->biography }}</p>@endif
                    @if($canShowTeacherProfile)<a href="{{ route('teachers.show', $course->teacher) }}" class="mt-4 inline-flex text-xs font-black text-green-700 no-underline">@lang('teacher-course::reviews.view_profile') →</a>@endif
                </section>
                <section class="rounded-xl border border-slate-200 bg-white p-5">
                    ...
                    <dl class="mt-3 divide-y divide-slate-100 text-xs">
                        @foreach(['subject' => $course->subject?->name, 'category' => $course->category?->name, 'education_level' => __('teacher-course::app.education_levels.'.$course->education_level), 'difficulty' => __('teacher-course::app.difficulties.'.$course->difficulty), 'language' => strtoupper($course->language), 'starts_at' => $course->starts_at?->format('d/m/Y'), 'schedule_days' => collect($course->schedule_days ?? [])->map(fn (string $day) => __('teacher-course::app.schedule_days.'.$day))->join(', '), 'study_time' => $course->study_time, 'students' => number_format((int) $course->enrollment_count)] as $label => $value)
