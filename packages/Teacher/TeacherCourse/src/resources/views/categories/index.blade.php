...
<div class="min-h-screen bg-slate-50">
    ...
    <main class="space-y-4 p-4 sm:p-6">
        ...
        <section class="overflow-hidden rounded-xl border border-slate-200 bg-white">
            <div class="overflow-x-auto"><table class="w-full min-w-[680px] text-left text-sm"><thead class="bg-slate-50 text-[10px] font-black uppercase text-slate-400"><tr><th class="px-5 py-3">@lang('teacher-course::categories.name')</th><th class="px-5 py-3">@lang('teacher-course::categories.order')</th><th class="px-5 py-3">@lang('teacher-course::categories.courses')</th><th class="px-5 py-3">@lang('teacher-course::categories.status')</th><th class="px-5 py-3"></th></tr></thead><tbody class="divide-y divide-slate-100">@forelse($categories as $category)<tr><td class="px-5 py-3"><strong>{{ $category->name }}</strong><span class="mt-1 block text-xs text-slate-400">{{ $category->description }}</span></td><td class="px-5 py-3">{{ $category->sort_order }}</td><td class="px-5 py-3">{{ $category->courses_count }}</td><td class="px-5 py-3"><span class="rounded-full px-2.5 py-1 text-xs font-black {{ $category->is_active ? 'bg-green-50 text-green-700' : 'bg-slate-100 text-slate-500' }}">{{ __($category->is_active ? 'teacher-course::categories.active' : 'teacher-course::categories.inactive') }}</span></td><td class="px-5 py-3"><div class="flex justify-end gap-2"><a href="{{ route('admin.course-categories.edit', $category) }}" class="rounded-lg border border-slate-200 px-3 py-2 text-xs font-black text-slate-600 no-underline">@lang('teacher-course::categories.edit')</a><form method="POST" action="{{ route('admin.course-categories.destroy', $category) }}" data-mindigo-confirm-title="@lang('teacher-course::categories.delete')" data-mindigo-confirm-message="@lang('teacher-course::categories.delete_confirm')" data-mindigo-confirm-text="@lang('teacher-course::categories.delete')" data-mindigo-confirm-cancel="@lang('teacher-course::categories.cancel')" data-mindigo-confirm-type="danger">@csrf @method('DELETE')<button class="rounded-lg bg-red-50 px-3 py-2 text-xs font-black text-red-700">@lang('teacher-course::categories.delete')</button></form></div></td></tr>@empty<tr><td colspan="5" class="p-10 text-center text-sm font-semibold text-slate-400">@lang('teacher-course::categories.empty')</td></tr>@endforelse</tbody></table></div>
