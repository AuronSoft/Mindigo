...
<div class="flex h-screen flex-col bg-slate-900">
    ...
    <header class="flex items-center justify-between gap-4 border-b border-slate-800 bg-slate-900 px-5 py-3">
        <div class="flex items-center gap-3 min-w-0">
            ...
            <div class="min-w-0">
                ...
                <p class="truncate text-xs font-semibold text-slate-400">{{ $session->classroom->name ?? '' }}</p>
            </div>
        </div>
        <div class="flex items-center gap-2">
            <a href="{{ route('teacher.live-sessions.index') }}"
               class="inline-flex h-9 items-center gap-1.5 rounded-full border border-slate-700 bg-slate-800 px-4 text-xs font-black text-slate-200 no-underline transition hover:bg-slate-700">
                ...
                @lang('teacher-live-session::app.leave_room')
            </a>
            <form action="{{ route('teacher.live-sessions.end', $session) }}" method="POST"
                ...
                @csrf
                <button type="submit"
                    class="inline-flex h-9 items-center gap-1.5 rounded-full bg-red-600 px-4 text-xs font-black text-white transition hover:bg-red-500">
                    <x-heroicon-o-stop class="h-4 w-4" />
                    @lang('teacher-live-session::app.end')
                </button>
            </form>
        </div>
    </header>
    ...
    <div id="jitsi-root" class="relative flex-1">
        <div id="jitsi-loading" class="absolute inset-0 flex flex-col items-center justify-center gap-3 text-slate-400">
            ...
            <p class="text-sm font-bold">@lang('teacher-live-session::app.room_loading')</p>
        </div>
    </div>
</div>
...
<script>
(function () {
    const indexUrl = @json(route('teacher.live-sessions.index'));

    function launch() {
        if (typeof JitsiMeetExternalAPI === 'undefined') {
            // Thư viện chưa tải xong (mất mạng) — thử lại sau 1s
        ...
        // Khi GV bấm "rời cuộc" trong Jitsi → quay về danh sách
