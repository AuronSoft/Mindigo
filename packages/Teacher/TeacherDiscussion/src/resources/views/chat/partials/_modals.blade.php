...
<div id="discussion-view-members-modal" data-discussion-modal class="fixed inset-0 z-50 hidden items-center justify-center bg-slate-900/50 p-4">
    <div class="flex max-h-[85vh] w-full max-w-md flex-col overflow-hidden rounded-3xl bg-white shadow-2xl">
        <div class="flex items-center justify-between border-b border-slate-100 px-6 py-4">
        ...
        @if($canManage)
            <div class="border-b border-slate-100 px-6 py-4">
                <form method="POST" action="{{ route($routes['membersStore'], $selectedThread->id) }}">
                    @csrf
                    <div class="mb-2 flex items-center justify-between">
                        <label class="text-xs font-black text-slate-600">@lang('teacher-discussion::app.add_members')</label>
                        <span class="text-[11px] font-bold text-slate-400" id="discussion-add-member-count">0 @lang('teacher-discussion::app.selected')</span>
                    </div>
                    <div class="mb-2 flex h-10 items-center gap-2 rounded-xl border border-slate-200 bg-white px-3">
                        <x-heroicon-o-magnifying-glass class="h-4 w-4 shrink-0 text-slate-400" />
                    ...
                    <button type="submit" class="mt-3 w-full rounded-xl bg-green-600 px-4 py-2.5 text-xs font-black text-white shadow-sm shadow-green-600/20 transition hover:bg-green-500">@lang('teacher-discussion::app.add_member_btn')</button>
                </form>
            </div>
        @endif
                ...
                <div class="flex items-center gap-3 rounded-xl px-2 py-2 transition hover:bg-slate-50">
                    ...
                    @if($canManage && !$ownerUserIds->contains((int) $member->id))
                        <form method="POST" action="{{ route($routes['membersDestroy'], [$selectedThread, $member->id]) }}">
                            @csrf
                            @method('DELETE')
                            <button type="submit" title="@lang('teacher-discussion::app.remove_member')" class="grid h-8 w-8 shrink-0 place-items-center rounded-lg text-slate-400 transition hover:bg-red-50 hover:text-red-600">
