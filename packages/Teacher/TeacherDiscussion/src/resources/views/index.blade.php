        ...
        <aside class="flex min-h-0 flex-col border-r border-slate-200 bg-white">
            <div class="shrink-0 border-b border-slate-100 p-4">
                ...
                <div class="mt-5 flex items-center justify-between">
                    <h1 class="text-sm font-black text-slate-950">Messages</h1>
                    <span class="text-[11px] font-black uppercase tracking-wider text-slate-400">@lang('teacher-discussion::app.scope')</span>
                </div>
            ...
            <div class="min-h-0 flex-1 overflow-y-auto p-3">
                @forelse($threads as $thread)
                    @php
                        ...
                        $searchText = mb_strtolower($className . ' ' . ($latest?->body ?? ''));
                    @endphp
                        ...
                        <div class="min-w-0 flex-1">
                            <div class="flex items-center gap-2">
                                ...
                                <span class="shrink-0 text-[10px] font-bold text-slate-400">
                                    {{ $thread->last_message_at?->diffForHumans() ?? 'Now' }}
                                </span>
                            </div>
                        ...
                        <div class="flex min-w-0 items-center gap-3">
                            ...
                            <div class="min-w-0">
                                ...
                                <p class="truncate text-xs font-bold text-slate-400">
                                    {{ number_format($selectedThread->classroom?->students_count ?? 0) }} @lang('teacher-discussion::app.students')
                                </p>
                            </div>
                        </div>
                                        ...
                                        @if($message->attachments->isNotEmpty())
                                            <div class="grid gap-2 {{ $message->attachments->filter(fn ($attachment) => $attachment->isImage())->count() > 1 ? 'grid-cols-2' : '' }}">
                                                @foreach($message->attachments as $attachment)
                                                    ...
                                                    @else
                                                        <a href="{{ $attachment->url() }}" target="_blank" class="flex items-center gap-3 rounded-2xl {{ $mine ? 'bg-white/15 text-white' : 'bg-white text-slate-700' }} p-3 no-underline">
                ...
                <form method="POST" action="{{ route('teacher.discussions.messages.store', $selectedThread) }}" enctype="multipart/form-data" class="shrink-0 border-t border-slate-100 bg-white px-5 py-3">
                    ...
                    <div class="mx-auto max-w-3xl">
                        ...
                        <div class="flex items-end gap-2 rounded-2xl border border-slate-200 bg-white p-2">
                            ...
                            <button type="button" class="grid h-10 w-10 shrink-0 place-items-center rounded-xl text-slate-400 transition hover:bg-slate-50 hover:text-green-700" data-discussion-voice title="@lang('teacher-discussion::app.voice_input')">
        ...
        @if($selectedThread)
            <aside class="min-h-0 overflow-y-auto border-l border-slate-200 bg-white max-xl:absolute max-xl:bottom-0 max-xl:right-0 max-xl:top-0 max-xl:z-30 max-xl:hidden max-xl:w-80 max-xl:shadow-2xl" data-discussion-info-panel>
                <div class="border-b border-slate-100 p-5">
                    <h2 class="text-sm font-black text-slate-950">Group Information</h2>
                </div>
                ...
                <div class="p-5 text-center">
                    ...
                    <p class="mt-1 text-xs font-bold text-slate-400">{{ number_format($members->count()) }} members</p>

                    <div class="mt-5 grid grid-cols-4 gap-2">
                        @foreach([
                            ['icon' => 'heroicon-o-bell', 'label' => 'Notify'],
                            ['icon' => 'heroicon-o-map-pin', 'label' => 'Pin'],
                            ['icon' => 'heroicon-o-plus', 'label' => 'Member'],
                            ['icon' => 'heroicon-o-cog-6-tooth', 'label' => 'Setting'],
                        ] as $action)
                            <button type="button" class="flex flex-col items-center gap-1 rounded-xl border border-slate-200 bg-white px-2 py-2 text-[10px] font-black text-slate-500 transition hover:bg-slate-50">
                ...
                <div class="space-y-6 px-5 pb-6">
                    <section>
                        <div class="mb-3 flex items-center justify-between">
                            <h3 class="text-sm font-black text-slate-950">Members</h3>
                            <span class="text-xs font-black text-green-700">View All</span>
                        </div>
                        <div class="space-y-2">
                    ...
                    <section>
                        <div class="mb-3 flex items-center justify-between">
                            <h3 class="text-sm font-black text-slate-950">Images</h3>
                            <span class="text-xs font-black text-green-700">View All</span>
                        </div>
                        @if($imageAttachments->isNotEmpty())
                            <div class="grid grid-cols-3 gap-2">
                                @foreach($imageAttachments->take(6) as $attachment)
                                    <a href="{{ $attachment->url() }}" target="_blank" class="block overflow-hidden rounded-xl bg-slate-100">
                                        <img src="{{ $attachment->url() }}" alt="{{ $attachment->original_name }}" class="aspect-square w-full object-cover">
                                    </a>
                                @endforeach
                            </div>
                        @else
                    ...
                    <section>
                        <div class="mb-3 flex items-center justify-between">
                            <h3 class="text-sm font-black text-slate-950">Files</h3>
                            <span class="text-xs font-black text-green-700">View All</span>
                        </div>
                        <div class="space-y-2">
                            @forelse($fileAttachments->take(5) as $attachment)
                                <a href="{{ $attachment->url() }}" target="_blank" class="flex items-center gap-3 rounded-xl p-2 no-underline transition hover:bg-slate-50">
                    ...
                    <section>
                        <div class="mb-3 flex items-center justify-between">
                            <h3 class="text-sm font-black text-slate-950">Links</h3>
                            <span class="text-xs font-black text-green-700">View All</span>
                        </div>
                        <div class="space-y-2">
                            ...
                            @empty
                                <p class="rounded-xl bg-slate-50 p-3 text-center text-xs font-bold text-slate-400">Links shared in chat will show here.</p>
                            @endforelse
                        </div>
                    </section>
                </div>
