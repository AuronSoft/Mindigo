...
<main class="min-h-screen bg-stone-50 p-4 lg:h-screen lg:overflow-hidden lg:p-5">
    <div class="grid h-full w-full gap-4 lg:grid-cols-12">
        ...
        <aside class="flex min-h-0 flex-col gap-4 lg:col-span-3">
            ...
            <section class="min-h-0 flex-1 rounded-2xl border border-slate-200 bg-white p-4"><div class="flex items-center justify-between"><h2 class="text-sm font-black text-slate-950">@lang('student-dashboard::app.planned_today')</h2>@if(Route::has('student.schedule.index'))<a href="{{ route('student.schedule.index') }}" class="grid h-8 w-8 place-items-center rounded-full bg-green-600 text-white"><x-heroicon-o-calendar-days class="h-4 w-4" /></a>@endif</div><div class="mt-3 space-y-2">@forelse(($todayTasks->isNotEmpty() ? $todayTasks : $activeTasks->take(3)) as $task)<a href="{{ $task->url ?? '#' }}" class="flex items-center gap-3 rounded-2xl border border-slate-100 p-3 no-underline transition hover:border-green-200 hover:bg-green-50/40"><span class="text-xs font-black text-slate-800">{{ $task->at?->format('H:i') }}</span><div class="min-w-0 border-l border-slate-200 pl-3"><h3 class="truncate text-xs font-black text-slate-800">{{ $task->title }}</h3><p class="mt-0.5 flex items-center gap-1.5 text-[10px] font-semibold text-green-600">{{ $task->status }}@if(!empty($task->provider))<span class="rounded-full bg-slate-100 px-1.5 py-0.5 text-[9px] font-black text-slate-500">{{ str($task->provider)->replace('_', ' ')->title() }}</span>@endif</p></div></a>@empty<p class="py-8 text-center text-xs font-semibold text-slate-400">@lang('student-dashboard::app.nothing_today')</p>@endforelse</div>
