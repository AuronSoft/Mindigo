...
<main class="min-h-screen bg-stone-50 p-4 lg:h-screen lg:overflow-hidden lg:p-5">
    <div class="mx-auto grid h-full max-w-[1800px] gap-4 lg:grid-cols-[minmax(15rem,0.82fr)_minmax(30rem,1.75fr)_minmax(16rem,0.9fr)]">
        ...
        <section class="flex min-h-0 flex-col gap-4">
            <article class="flex items-center justify-between gap-5 rounded-2xl border border-green-100 bg-gradient-to-r from-green-50 to-white px-6 py-5">
            ...
            <section class="min-h-0 flex-1 rounded-2xl border border-slate-200 bg-white p-4">
                ...
                <div class="mt-3 divide-y divide-slate-100">@forelse($assignments as $assignment)<article class="grid grid-cols-[minmax(0,1fr)_5rem_7rem] items-center gap-4 py-3"><div class="flex min-w-0 items-center gap-3"><span class="grid h-9 w-9 shrink-0 place-items-center rounded-xl bg-emerald-50 text-emerald-600"><x-heroicon-o-book-open class="h-4 w-4" /></span><div class="min-w-0"><h3 class="truncate text-xs font-black text-slate-800">{{ $assignment->title }}</h3><p class="mt-0.5 truncate text-[10px] font-semibold text-slate-400">{{ $assignment->status }}</p></div></div><span class="text-[10px] font-bold text-slate-400">{{ $assignment->at?->format('d/m') }}</span><div><progress value="{{ max(0, 100 - now()->diffInHours($assignment->at, false)) }}" max="100" class="h-1.5 w-full accent-green-600"></progress><p class="mt-1 text-right text-[9px] font-black text-green-600">{{ $assignment->time_left }}</p></div></article>@empty<p class="py-12 text-center text-xs font-semibold text-slate-400">@lang('student-dashboard::app.empty_tasks')</p>@endforelse</div>
            </section>
        </section>
