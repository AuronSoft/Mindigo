...
@endphp
<div class="student-calendar-shell bg-slate-50 text-slate-900" data-student-calendar-workspace>
    <header class="student-calendar-header border-b border-slate-200 bg-white px-4 py-3 lg:px-5">
        <div class="flex flex-wrap items-center justify-between gap-4">
            ...
            <nav aria-label="Calendar views" class="flex rounded-xl border border-slate-200 bg-slate-50 p-1">
                @foreach(['today','week','month','schedule'] as $mode)<a href="{{ route('student.schedule.index', [...$query, 'view' => $mode, 'date' => $anchor->toDateString()]) }}" class="rounded-lg px-3 py-2 text-xs font-black no-underline {{ $viewMode === $mode ? 'bg-green-700 text-white shadow-sm' : 'text-slate-500 hover:text-green-700' }}">@lang('student-schedule::app.view_'.$mode)</a>@endforeach
            </nav>
    ...
    <main class="student-calendar-body">
        <section aria-labelledby="next-actions" class="student-calendar-sidebar"><div class="mb-3"><div><h2 id="next-actions" class="text-sm font-black">@lang('student-schedule::app.next_actions')</h2><p class="mt-1 text-[10px] font-semibold text-slate-400">@lang('student-schedule::app.next_actions_hint')</p></div><span class="mt-2 block text-[10px] font-black text-green-700">{{ $summary['events'] }} @lang('student-schedule::app.events')</span></div><div class="flex flex-col gap-2">
            @foreach(['session' => CalendarEventKind::ClassSession, 'assignment' => CalendarEventKind::AssignmentDue, 'exam' => CalendarEventKind::ExamWindow] as $slot => $kind)
                ...
                <button type="button" @if($event) data-student-event='@json($payload($event))' @endif class="student-calendar-event rounded-xl border border-slate-200 bg-white p-3 text-left {{ $event ? 'hover:border-green-300' : 'cursor-default' }}"><div class="flex items-start gap-2.5"><span class="grid h-9 w-9 shrink-0 place-items-center rounded-lg {{ $tones[$kind->value] }}"><x-dynamic-component :component="$icons[$kind->value]" class="h-4 w-4" /></span><span class="min-w-0"><small class="font-black uppercase tracking-wide text-slate-400">@lang('student-schedule::app.next_'.$slot)</small>@if($event)<strong class="mt-1 block truncate text-xs">{{ $event->title }}</strong><span class="mt-1 block text-[10px] font-bold text-slate-500">{{ $event->startsAt->format('d/m/Y · H:i') }} · {{ $event->metadata['classroom_name'] ?? __('student-schedule::app.personal') }}</span>@else<span class="mt-2 block text-[10px] font-semibold text-slate-400">@lang('student-schedule::app.no_upcoming')</span>@endif</span></div></button>
            @endforeach
        </div></section>
