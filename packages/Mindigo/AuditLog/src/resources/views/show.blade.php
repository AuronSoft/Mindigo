...
@section('title', __('Mindigo-audit-log::app.detail_title'))
...
@section('content')
    <div class="mx-auto flex max-w-6xl flex-col gap-6">
            ...
            <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                <div>
                    <div class="flex items-center gap-1.5 text-xs font-black text-slate-400">
                        ...
                        <a href="{{ route('audit-logs.index') }}" class="text-slate-500 no-underline transition hover:text-green-700">@lang('Mindigo-audit-log::app.breadcrumb')</a>
                        <svg class="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5"><polyline points="9 18 15 12 9 6"/></svg>
                        <span class="text-slate-700">#{{ $log->id }}</span>
                    </div>
                    <h1 class="mt-2 text-2xl font-black tracking-tight text-slate-950">@lang('Mindigo-audit-log::app.detail') #{{ $log->id }}</h1>
                    <p class="mt-1 text-sm font-semibold leading-6 text-slate-500">{{ $log->created_at?->format('d/m/Y H:i:s') }}</p>
                </div>
                <a href="{{ route('audit-logs.index') }}" class="inline-flex min-h-11 items-center justify-center rounded-xl border border-slate-200 px-4 text-sm font-black text-slate-600 no-underline transition hover:bg-slate-50">@lang('Mindigo-audit-log::app.back')</a>
            </div>
        </header>

        <section class="grid gap-4 lg:grid-cols-3">
            <div class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm shadow-slate-200/70">
                <div class="text-xs font-black uppercase text-slate-400">@lang('Mindigo-audit-log::app.user')</div>
                <div class="mt-2 text-base font-black text-slate-950">{{ $log->user_name ?? __('Mindigo-audit-log::app.guest') }}</div>
                <div class="mt-1 text-sm font-semibold text-slate-500">{{ $log->user_email ?? '-' }}</div>
            </div>
            <div class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm shadow-slate-200/70">
                <div class="text-xs font-black uppercase text-slate-400">@lang('Mindigo-audit-log::app.action')</div>
            ...
            <div class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm shadow-slate-200/70">
                <div class="text-xs font-black uppercase text-slate-400">@lang('Mindigo-audit-log::app.request')</div>
        ...
        <section class="grid gap-4 lg:grid-cols-2">
            <div class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm shadow-slate-200/70">
                <div class="flex items-center justify-between gap-3">
                    <h2 class="text-sm font-black text-slate-950">@lang('Mindigo-audit-log::app.old_values')</h2>
                    <button type="button" class="rounded-lg border border-slate-200 px-2.5 py-1 text-xs font-black text-slate-500 transition hover:bg-slate-50 hover:text-green-700" data-audit-copy="#audit-old-values">@lang('Mindigo-audit-log::app.copy')</button>
                </div>
                <pre id="audit-old-values" class="audit-log-json mt-3" data-audit-json="old">{{ $formatJson($log->old_values) }}</pre>
            </div>
            <div class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm shadow-slate-200/70">
                <div class="flex items-center justify-between gap-3">
                    <h2 class="text-sm font-black text-slate-950">@lang('Mindigo-audit-log::app.new_values')</h2>
                    <button type="button" class="rounded-lg border border-slate-200 px-2.5 py-1 text-xs font-black text-slate-500 transition hover:bg-slate-50 hover:text-green-700" data-audit-copy="#audit-new-values">@lang('Mindigo-audit-log::app.copy')</button>
                </div>
        ...
        <section class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm shadow-slate-200/70">
            <div class="flex items-center justify-between gap-3">
                <h2 class="text-sm font-black text-slate-950">@lang('Mindigo-audit-log::app.metadata')</h2>
                <button type="button" class="rounded-lg border border-slate-200 px-2.5 py-1 text-xs font-black text-slate-500 transition hover:bg-slate-50 hover:text-green-700" data-audit-copy="#audit-metadata">@lang('Mindigo-audit-log::app.copy')</button>
            </div>
