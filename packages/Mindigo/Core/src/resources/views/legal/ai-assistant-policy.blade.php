...
<div class="min-h-screen bg-slate-50 text-slate-800">
    <header class="relative overflow-hidden bg-emerald-950">
        <div class="absolute inset-0 opacity-20">
            <img
                src="https://images.unsplash.com/photo-1677442136019-21780ecad995?w=1800&q=85"
                alt=""
        ...
        <div class="relative mx-auto max-w-7xl px-6 pb-20 pt-16 lg:px-10 lg:pb-24">
            <div class="max-w-4xl">
                ...
                <div class="mt-8 flex flex-wrap gap-3">
                    <span class="rounded-lg border border-white/25 bg-white/10 px-4 py-3 text-sm font-black text-white">{{ $policy['meta']['effective_date'] }}</span>
                    <span class="rounded-lg border border-white/25 bg-white/10 px-4 py-3 text-sm font-black text-white">{{ $policy['meta']['version'] }}</span>
                    <span class="rounded-lg border border-white/25 bg-white/10 px-4 py-3 text-sm font-black text-white">{{ $policy['meta']['scope'] }}</span>
                </div>
            </div>
        </div>
    </header>
    ...
    <main class="mx-auto grid max-w-7xl gap-8 px-6 py-12 lg:grid-cols-[320px_1fr] lg:px-10">
        <aside class="lg:sticky lg:top-6 lg:self-start">
            <nav class="max-h-[calc(100vh-3rem)] overflow-auto rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
                <p class="mb-4 text-sm font-black uppercase tracking-[0.14em] text-emerald-950">{{ $policy['toc_title'] }}</p>
                <div class="space-y-1">
    ...
    <footer class="border-t border-slate-200 bg-white px-6 py-8 lg:px-10">
        <div class="mx-auto flex max-w-7xl flex-col gap-4 text-sm text-slate-500 md:flex-row md:items-center md:justify-between">
            <p>{{ $policy['footer']['copyright'] }}</p>
            <div class="flex flex-wrap gap-4">
                <a href="{{ route('home', [], false) }}" class="font-bold text-slate-600 hover:text-emerald-700">{{ $policy['footer']['home'] }}</a>
                <a href="{{ route('terms', [], false) }}" class="font-bold text-slate-600 hover:text-emerald-700">{{ $policy['footer']['terms'] }}</a>
                <a href="{{ route('privacy', [], false) }}" class="font-bold text-slate-600 hover:text-emerald-700">{{ $policy['footer']['privacy'] }}</a>
            </div>
        </div>
    </footer>
</div>
