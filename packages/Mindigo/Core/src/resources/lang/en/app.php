<?php

return [
    'processing' => [
        'label' => 'The system is processing',
        'loading' => 'Loading content...',
        'please_wait' => 'Please wait a moment',
    ],
    'meta' => ['description' => 'Mindigo learning management platform for courses, practice, and exams.'],
    'trust' => [
        'heading_1' => 'Trusted by students, universities and',
        'heading_2' => 'businesses across the country',
        'student' => [
            'title' => 'Students',
            'desc' => 'A platform for sharing exams and study materials. By creating their own tests, students can self-study with suitable resources while sharing them with their study groups.',
            'cta' => 'Get started',
        ],
        'lecturer' => [
            'title' => 'Lecturers',
            'desc' => 'Simple and accurate exam creation with effective assessment methods, helping lecturers easily manage lessons and teaching quality.',
            'cta' => 'Get started',
        ],
        'training' => [
            'title' => 'Training Centers',
            'desc' => 'A technically detailed platform that helps businesses quickly organize and arrange training content for their employees.',
            'cta' => 'Get started',
        ],
    ],

    'testimonials' => [
        'title' => 'Customer Feedback',
        'customers' => '200,000+ customers',
        'read_more' => 'Read more',
        'community_title_1' => 'Join the biggest',
        'community_title_2' => 'community of learning',
        'community_title_prefix' => 'Join the biggest',
        'community_title_accent' => 'community',
        'community_title_suffix' => 'of learning',
        'community_description' => 'Learn alongside a growing global community, exchange knowledge and build meaningful progress together on Mindigo.',
        'community_map_alt' => 'Mindigo learners connected around the world',
        'learners' => 'Learners',
        'join_now' => 'Join now',
        'member_avatar' => 'Mindigo community member',
        'reviews' => [
            [
                'name' => 'Nguyễn Thư',
                'school' => 'Thuong Mai University',
                'seed' => 'Thu',
                'review' => 'I have been using Mindigo and I am really satisfied. The exam feature saves a lot of study time. Overall, the diverse exercises and intuitive interface make learning more enjoyable.',
            ],
            [
                'name' => 'Gia Khánh',
                'school' => 'Thuong Mai University',
                'seed' => 'Khanh',
                'review' => 'I have been using Mindigo for a long time and I am truly satisfied. The mock exam feature saves me a lot of revision time, the exercises are diverse, and the intuitive interface makes studying more effective.',
            ],
            [
                'name' => 'Nguyễn Hà',
                'school' => 'Le Quy Don High School - Ha Dong',
                'seed' => 'Ha',
                'review' => 'This software is really great, it helps me study and improve my memory skills, and it is very easy to use for everyone. Overall it is very useful, students should use it.',
            ],
            [
                'name' => 'Hùng Mai',
                'school' => 'Hanoi University of Business and Technology',
                'seed' => 'Hung',
                'review' => 'While using Mindigo to study for exams, it helped me easily memorize knowledge that was originally very academic. Moreover, I had access to a diverse set of exam questions and an intuitive interface.',
            ],
            [
                'name' => 'Su Trà',
                'school' => 'Posts and Telecommunications Institute of Technology',
                'seed' => 'Su',
                'review' => 'I recently discovered Mindigo while searching for practice exams. Mindigo has helped me a lot in finding answers and questions on many different topics.',
            ],
            [
                'name' => 'Minh Tuấn',
                'school' => 'Hanoi University of Science and Technology',
                'seed' => 'Tuan',
                'review' => 'Mindigo is truly a great learning tool. I especially like the automatic exam generation feature from documents, saving a lot of preparation time for exams.',
            ],
        ],
    ],

    'navbar' => [
        'features' => 'Features',
        'explore' => 'Explore Exams',
        'become_teacher' => 'Teach with Mindigo',
        'exam_prep' => 'High School Exam Prep',
        'classroom' => 'Classroom Management',
        'pricing' => 'Pricing',
        'news' => 'News',
        'contact' => 'Contact',
        'login' => 'Login',
        'mega' => [
            'eyebrow' => 'Learning management platform',
            'title' => 'Every LMS feature in one system',
            'subtitle' => 'Create content, organize training and measure learning outcomes from one place.',
            'overview' => 'Feature overview',
            'items' => [
                ['title' => 'Courses & content', 'desc' => 'Create lessons, video and manage multimedia resources.'],
                ['title' => 'Classroom management', 'desc' => 'Organize classes, learners, schedules and announcements.'],
                ['title' => 'Question bank', 'desc' => 'Build, categorize and reuse your question library.'],
                ['title' => 'Exams & practice', 'desc' => 'Create tests, shuffle questions, grade and deter cheating.'],
                ['title' => 'Assignments & assessment', 'desc' => 'Assign work, collect submissions and provide feedback.'],
                ['title' => 'Live interaction', 'desc' => 'Discussions, live classes and two-way communication.'],
                ['title' => 'Progress & reports', 'desc' => 'Track results, learning time and training performance.'],
                ['title' => 'Users & permissions', 'desc' => 'Manage accounts, roles and secure access permissions.'],
                ['title' => 'Learning paths', 'desc' => 'Design programs by learner, role and learning objective.'],
                ['title' => 'Content library', 'desc' => 'A shared library of learning resources and training programs.'],
                ['title' => 'Competency framework', 'desc' => 'Map role competencies and personal development plans.'],
                ['title' => 'Gamification', 'desc' => 'Motivate learning with points, badges and leaderboards.'],
            ],
            'highlights' => [
                ['title' => 'Gemini assistant', 'desc' => 'Suggest content and help generate exams faster.'],
                ['title' => 'Learn on any device', 'desc' => 'Optimized for desktop, tablet and mobile.'],
                ['title' => 'Data security', 'desc' => 'Authentication, permissions and learning data protection.'],
                ['title' => 'Custom branding', 'desc' => 'Customize organization logo, colors and domain.'],
            ],
        ],
    ],

    'hero' => [
        'badge' => '#1 LMS platform for online training',
        'heading_1' => 'A simpler way to',
        'heading_2' => 'manage learning',
        'heading_3' => 'with one modern LMS',
        'heading_4' => 'build courses',
        'heading_5' => 'track progress',
        'floating_instructor' => 'Learning mentor',
        'floating_instructor_name' => 'Minh Nguyen',
        'floating_live_class' => 'Live class is ready',
        'floating_join' => 'Join class',
        'floating_profile' => 'View profile',
        'floating_active' => 'Active',
        'floating_students' => 'Learners',
        'floating_students_joined' => '128K+ learners',
        'floating_progress' => 'Course progress',
        'floating_online' => '128 learners online',
        'desc' => 'Mindigo helps organizations build courses, manage classes, distribute learning materials and track learner progress in one intuitive LMS.',
        'customers' => 'Over <strong class="text-green-600">200,000+</strong> customers love using it',
        'cta_create' => 'Start your LMS',
        'cta_search' => 'Explore courses',
        'ai_powered' => 'Powered by AI',
        'ai_subtitle' => '✦ Learning path suggestions',
        'advanced' => 'Advanced Features',
        'url_display' => 'app.mindigo.vn/lms/courses',
        'upload' => 'Upload course material',
        'review' => 'Review results',
        'return' => '↩ Return',
        'save' => '✓ Save course',
        'section_list' => 'Learning modules',
        'part_1' => 'Module 1',
        'part_2' => 'Module 2: Advanced',
        'question_index' => 'Lesson index (10)',
        'progress' => 'Progress',
        'progress_done' => '3/10 lessons completed',
        'question_list' => 'Lesson List',
        'q1_title' => 'Lesson 1',
        'q1_type' => 'Video',
        'q2_title' => 'Lesson 2',
        'q2_type' => 'Material',
        'has_answer' => '✓ Published',
        'course_name' => 'New Employee Onboarding Program',
        'course_meta' => '248 learners · 6 active cohorts',
        'lesson_1_name' => 'Welcome and Company Culture',
        'lesson_1_meta' => 'Interactive video · 12 minutes',
        'lesson_1_desc' => 'An introduction to the organization, core values, and onboarding journey.',
        'lesson_2_name' => 'Processes and Workplace Tools',
        'lesson_2_meta' => 'Learning guide · 8 pages',
        'lesson_2_desc' => 'Practical resources, checklists, and the learner’s first assignment.',
        'learners_label' => 'Learners',
        'completion_label' => 'Completion',
        'ai_processing' => 'AI is processing the material...',
        'save_success' => 'Course changes saved',
        'lesson_name_label' => 'Lesson name',
        'lesson_name_value' => 'Employee Onboarding Process',
        'phone_add_q' => 'Add Lesson',
        'q_type_label' => 'Content Type',
        'difficulty' => 'Level',
        'difficulty_med' => 'Medium',
        'q_types' => [
            'Single Answer',
            'Multiple Answers',
            'True/False',
            'Matching',
            'Fill in the blank',
            'Reading Comprehension',
            'Essay',
        ],
    ],

    'virtual_exam' => [
        'eyebrow' => 'Reach learners with high-quality live teaching',
        'headline_before' => 'Teach',
        'headline_highlight' => 'live online',
        'headline_after' => 'with everything in one place',
        'description' => 'Host engaging classes, connect with learners in real time and manage every interaction through one seamless Mindigo classroom.',
        'email_placeholder' => 'Your email address',
        'class_title' => 'Ms. Anna’s live learning room',
        'class_meta' => '18 August · 26:32 recording time',
        'recording' => 'Recording',
        'teacher_alt' => 'Teacher presenting a live online class',
        'teacher_name' => 'Ms. Anna',
        'participant_alt' => 'Live class participant',
        'participant_names' => ['Sara', 'James', 'John', 'You'],
        'controls' => ['microphone' => 'Microphone', 'camera' => 'Camera', 'screen' => 'Share screen'],
        'share' => 'Share lesson',
        'messages' => 'Messages',
        'messages_desc' => 'Chat with everyone during the class and keep the conversation connected.',
        'chat' => [
            ['name' => 'Can', 'time' => '09:23', 'message' => 'Hello, how is everyone doing?'],
            ['name' => 'Melissa', 'time' => '09:25', 'message' => 'The lesson is very clear, thank you!'],
            ['name' => 'Selin', 'time' => '09:26', 'message' => 'Could you explain this example again?'],
            ['name' => 'Anna', 'time' => '09:28', 'message' => 'Of course — let us review it together.'],
        ],
        'message_placeholder' => 'Write a message...',
        'send' => 'Send message',
        'badge' => 'EFFECTIVE',
        'title_1' => 'Virtual Exam Room',
        'title_2' => 'online',
        'desc_1' => 'The platform allows organizing exams and tests in a safe, secure, and efficient manner. Candidates can take exams from anywhere with an internet connection, without needing to travel to a specific exam location.',
        'desc_2' => 'Mindigo uses advanced technology to simulate traditional exam rooms while delivering superior benefits compared to conventional methods.',
        'cta' => 'Get Started',
        'url_room' => 'app.mindigo.vn/exam-room',
        'url_manage' => 'app.mindigo.vn/manage-exams',
        'exam_name' => 'Midterm Exam 1',
        'live' => '● In Progress',
        'enter' => 'Enter Exam',
        'search_candidate' => 'Search candidates...',
        'grid_view' => 'Grid',
        'list_view' => 'List',
        'room' => 'Room :number',
        'candidates' => '30 candidates',
        'status_live' => 'In Progress',
        'status_soon' => 'Upcoming',
        'status_stop' => 'Stopped',
        'status_end' => 'Ended',
        'manage_title' => 'Manage Exams',
        'search_exam' => 'Search exams...',
        'add_new' => 'Add New',
        'table_headers' => [
            'Exam Name',
            'Time',
            'Questions',
            'Joined',
            'Completed',
            'Status',
            'Actions',
        ],
        'pagination' => 'Rows per page: <strong>10</strong> · total: <strong>10</strong>',
        'exams' => [
            ['name' => 'General Review - Term 1',   'date' => '15/07/2024 - 20/07/2024', 'questions' => '7',  'joined' => '119', 'done' => '98',  'status' => 'In Progress', 'color' => 'green'],
            ['name' => 'Real Exam - National HS',   'date' => '10/12/2024 - 11/12/2024', 'questions' => '40', 'joined' => '0',   'done' => '4',   'status' => 'Upcoming',    'color' => 'blue'],
            ['name' => 'Test Round 1',              'date' => '05/09/2024 - 10/09/2024', 'questions' => '8',  'joined' => '143', 'done' => '0',   'status' => 'Upcoming',    'color' => 'blue'],
            ['name' => 'General Practice Exam',     'date' => '20/07/2024 - 30/08/2024', 'questions' => '7',  'joined' => '169', 'done' => '0',   'status' => 'Stopped',     'color' => 'orange'],
            ['name' => 'Advanced Review',           'date' => '12/09/2024 - 20/09/2024', 'questions' => '5',  'joined' => '103', 'done' => '148', 'status' => 'Ended',       'color' => 'red'],
        ],
        'rooms' => [
            ['bg' => 'bg-amber-100',  'number' => '1'],
            ['bg' => 'bg-green-100',  'number' => '2'],
            ['bg' => 'bg-blue-100',   'number' => '3'],
            ['bg' => 'bg-orange-100', 'number' => '4'],
        ],
        'subjects' => [
            ['name' => 'Mathematics', 'date' => '11/07/2024', 'total' => '120', 'done' => '115', 'btn_bg' => 'bg-green-500',  'label' => 'In Progress'],
            ['name' => 'Physics',     'date' => '10/12/2024', 'total' => '60',  'done' => '0',   'btn_bg' => 'bg-green-400',  'label' => 'Upcoming'],
            ['name' => 'Chemistry',   'date' => '05/09/2024', 'total' => '143', 'done' => '0',   'btn_bg' => 'bg-orange-400', 'label' => 'Stopped'],
        ],
    ],

    'personalization' => [
        'badge' => 'OPTIMIZED',
        'title_1' => 'Friendly and easy to use,',
        'title_2' => 'personalize',
        'title_3' => 'your learning',
        'desc_1' => 'Mindigo\'s interface is designed to be intuitive and user-friendly, helping users perform actions quickly and make the most of all useful features on the platform.',
        'desc_2' => 'Through personalization, each user has their own unique experience and builds a learning path that suits them. This stimulates independent thinking, creativity, and better academic results.',
        'cta' => 'Get Started',
        'label_easy' => 'Easy to use',
        'label_friendly' => 'Friendly',
        'phone1' => [
            'title' => 'Explore',
            'tab_exam' => 'Exams',
            'tab_course' => 'Courses',
            'search' => 'Search exams...',
            'filter_new' => 'Newest',
            'filter_hot' => 'Most Popular',
            'card1_subject' => 'CHEMISTRY EXAM',
            'card1_label' => 'THPTQG 2023',
            'card1_title' => 'National High School Chemistry Exam 2023 - Ministry of Education',
            'card1_q' => '61 questions',
            'card1_time' => '45 minutes',
            'card2_title' => 'National High School Exam Practice',
            'nav_explore' => 'Explore',
            'nav_exam' => 'Exams',
            'nav_class' => 'Classes',
        ],
        'phone2' => [
            'back' => 'Exam Details',
            'exam_title' => '50 PRONUNCIATION QUESTIONS',
            'exam_label' => 'THPTQG',
            'stat_views' => '3.4k',
            'stat_time' => '27min',
            'stat_q' => '50 questions',
            'btn_start' => 'Start',
            'desc' => 'A collection of 50 common pronunciation questions for THPTQG',
            'tab_content' => 'Exam Content',
            'tab_result' => 'Exam Results',
            'tab_list' => 'List',
            'part' => 'Part 1',
            'question' => 'Question 1',
            'q_preview' => 'Question 1: Choose the word...',
        ],
    ],

    'anytime_anywhere' => [
        'badge' => 'FLEXIBLE',
        'heading' => 'Learn anytime, anywhere',
        'feature_1' => 'No matter where you are, as long as you have an internet-connected device, you can study.',
        'feature_2' => 'You can learn on your smartphone, tablet, laptop, or desktop computer.',
        'cta' => 'Get started',
    ],

    'exam' => [
        'subject' => 'Advanced English for Specific Purposes',
        'topic' => 'Topic: Practice Test',
        'time_remaining' => 'Time remaining',
        'submit' => 'Submit',
        'calculator' => 'Calculator',
        'section_list' => 'Section list (2)',
        'section_1_title' => 'Part 1: Grammar and Vocabulary',
        'section_1_progress' => 'Progress: 3/15',
        'section_2_title' => 'Part 2: Reading Comprehension',
        'section_2_progress' => 'Progress: 0/15',
    ],

    'question' => [
        'label' => 'Question :number',
        'single_choice' => 'Single choice',
        'question_index' => 'Question navigator',
        'answered' => 'Answered',
        'flagged' => 'Flagged',
        'unanswered' => 'Unanswered',
    ],

    'mobile' => [
        'section_label' => 'Part 1',
        'progress' => '2/5',
        'question_label' => 'Question 2',
        'instruction' => 'Choose the correct answer for the question below...',
        'see_more' => 'Tap to see remaining options',
        'explanation' => 'Explanation: "were" is used with plural subjects in the past tense',
        'prev' => '← Previous',
        'next' => 'Next →',
    ],

    'nav' => [
        'url' => 'app.mindigo.vn/lam-bai-thi',
        'username' => 'Khánh Phương',
    ],

    'feature' => [
        'section_title' => 'A flexible and easy-to-use learning platform',
        'badge' => 'FAST',
        'heading_highlight' => 'Automatically generate',
        'heading_rest' => 'questions and multiple-choice exams',
        'feature_1' => 'Create exams quickly with just a few clicks. By uploading a WORD or PDF document, AI will help you generate 100% accurate exams in minutes.',
        'feature_2' => 'Optimise the experience, save time and effort, ensure objectivity, and free up more time for research and studying.',
        'cta' => 'Get started',
    ],

    'ai_card' => [
        'tab_input' => 'Enter text',
        'tab_review' => 'Review results',
        'placeholder' => 'Enter your exam text here...',
        'question_count_label' => 'Questions',
        'question_count_value' => '15 questions',
        'type_label' => 'Type',
        'type_value' => 'Single answer',
        'level_label' => 'Difficulty',
        'level_value' => 'Medium',
        'btn_return' => 'Back',
        'btn_save' => 'Save exam',
        'section_list_label' => 'Section list',
        'section_1_name' => 'Part 1: Vocabulary',
        'question_index_label' => 'Question navigator (19)',
        'question_list_label' => 'Question list',
        'question_1_label' => 'Question 1 (Single answer)',
        'question_2_label' => 'Question 2 (Single answer)',
        'generated_label' => 'Generated',
        'reviewed_label' => 'Reviewed',
        'accuracy_label' => 'AI accuracy',
        'ai_ready' => 'AI finished processing',
        'source_label' => 'Source',
        'source_value' => 'English textbook.pdf',
        'question_text' => 'She ___ to the store yesterday.',
        'option_a' => 'a. goes',
        'option_b' => 'b. went',
        'correct_answer' => 'Correct answer',
        'draft' => 'Draft',
        'video_meta' => 'Video · 08:45',
        'document_meta' => 'Document · PDF',
        'save_success' => 'Exam saved successfully',
        'save_success' => 'Exam saved successfully',
    ],

    'cta' => [
        'heading_1' => 'Operate a',
        'heading_2' => 'standard LMS',
        'heading_3' => 'for digital learning',
        'btn' => 'Start your LMS',
        'badge_1' => 'Manage classes and courses',
        'badge_2' => 'Track progress in realtime',
        'card_1_question' => 'Active learning program',
        'card_1_text' => 'Manage lessons, materials, learners and schedules in one connected workspace.',
        'card_1_a' => 'Lesson content',
        'card_1_b' => 'Training materials',
        'card_1_c' => 'Final assessment',
        'card_2_question' => 'Learner progress',
        'card_2_text' => 'Monitor completion, scores and engagement so every learner receives timely support.',
        'card_2_a' => 'Lesson completion',
        'card_2_b' => 'Average score',
        'card_2_c' => 'Needs support',
    ],

    'footer' => [
        'tagline' => 'An LMS platform for delivering training, managing learners and tracking learning progress effectively',
        'col_product' => 'Products & Services',
        'product_1' => 'Student exam prep',
        'product_2' => 'Exam organisation',
        'product_3' => 'National High School Exam prep',
        'col_resource' => 'Resources',
        'resource_1' => 'News',
        'resource_2' => 'Exam tips',
        'resource_3' => 'Study tools',
        'resource_4' => 'Exam archive',
        'resource_5' => 'National Exam study guide',
        'resource_6' => 'Community activities',
        'col_legal' => 'Terms & Policies',
        'legal_1' => 'Terms of use',
        'legal_2' => 'Privacy policy',
        'legal_3' => 'Technical support terms',
        'legal_4' => 'AI Assistant usage policy',
        'legal_5' => 'Refund policy',
        'legal_6' => 'Tutor policy',
        'copyright' => '© '.date('Y').' Auronsoft. All rights reserved.',
        'built_with' => 'Built with ❤️ in Vietnam',
    ],

    'contact' => [
        'section_title' => 'Contact',
        'header_badge' => 'CONTACT',
        'header_title' => 'We are always ready to listen to you',
        'header_title_highlight' => 'listen to you',
        'header_description' => 'Whether you have questions about our products, need technical support, or want to collaborate — the Mindigo team is always here to help you.',

        'form_title' => 'Send us a message',
        'label_name' => 'Full name',
        'placeholder_name' => 'John Doe',
        'label_email' => 'Email',
        'placeholder_email' => 'email@example.com',
        'label_phone' => 'Phone number',
        'placeholder_phone' => '0912 345 678',
        'label_subject' => 'Subject',
        'subject_technical' => 'Technical Support',
        'subject_product' => 'Product Consultation',
        'subject_business' => 'Business Partnership',
        'subject_bug' => 'Report a Bug',
        'subject_other' => 'Other',
        'label_message' => 'Message',
        'placeholder_message' => 'Enter your message here...',
        'button_send' => 'Send Message',

        'brand_tagline' => 'Vietnam\'s #1 Online Testing Platform',
        'brand_name' => 'Mindigo',
        'brand_description' => 'Helping over 200,000+ students, lecturers, and institutions create exams, practice, and manage classes more effectively every day.',
        'brand_users' => '200,000+ users',
        'brand_ai' => 'AI-powered',
        'brand_madein' => 'Made in Vietnam',

        'info_address' => 'Address',
        'address_detail' => '5th Floor, 97 Vo Van Tan, Ward 6, District 3, Ho Chi Minh City',
        'info_hotline' => 'Support Hotline',
        'hotline_number' => '1800 6868 (Toll-free)',
        'hotline_time' => 'Monday – Friday: 8:00 – 18:00',
        'info_email' => 'Email',
        'email_value' => 'support@mindigo.vn',
        'email_response' => 'Response within 24 hours',

        'map_title' => 'MAP',
        'map_heading' => 'Find us here',

        'faq_title' => 'FAQ',
        'faq_heading' => 'Frequently Asked Questions',
    ],

    'pricing' => [
        'section_title' => 'Pricing',
        'header_badge' => 'PRICING',
        'header_title' => 'Service plans that suit you',
        'header_title_plain' => 'Service plans',
        'header_title_highlight' => 'suit you',
        'header_description' => 'Start for free, upgrade when you\'re ready. No contracts, cancel anytime.',

        'toggle_monthly' => 'Monthly',
        'toggle_yearly' => 'Yearly',
        'save_percent' => '-20%',

        // Pricing Cards
        'free' => [
            'plan_name' => 'Free',
            'price' => '0đ',
            'period' => 'Forever free',
            'cta' => 'Start for free',
        ],
        'pro' => [
            'plan_name' => 'Pro',
            'badge' => 'MOST POPULAR',
            'price_monthly' => '199,000đ',
            'price_yearly' => '159,000đ',
            'period' => '/month',
            'yearly_note' => 'Pay 1,908,000đ/year — save 480,000đ',
            'monthly_note' => 'Billed monthly, cancel anytime',
            'cta' => 'Start 7-day free trial',
        ],
        'enterprise' => [
            'plan_name' => 'Enterprise',
            'price' => 'Contact us',
            'period' => 'Custom pricing for organizations',
            'cta' => 'Contact Sales',
        ],

        // Features
        'features' => [
            'unlimited_exams' => 'Unlimited exam creation',
            'unlimited_questions' => 'Unlimited questions',
            'unlimited_practice' => 'Unlimited practice attempts',
            'ai_generate' => 'AI Exam Generation (GPT-4)',
            'upload_document' => 'Upload Word/PDF → AI generates exam',
            'virtual_exam' => 'Unlimited virtual exam rooms',
            'class_management' => 'Classroom management',
            'priority_support' => 'Priority 24/7 support',
        ],

        'free_features' => [
            'max_5_exams' => 'Up to 5 exams per month',
            'max_30_questions' => 'Up to 30 questions per exam',
            'max_10_practice' => '10 practice attempts per month',
            'basic_quiz' => 'Basic quiz support',
        ],

        // Comparison Table
        'comparison' => [
            'title' => 'DETAILED COMPARISON',
            'heading' => 'Features by Plan',
            'feature' => 'Feature',
            'free_plan' => 'Free',
            'pro_plan' => 'Pro',
            'enterprise_plan' => 'Enterprise',
        ],

        // Trust Badges
        'trust' => [
            'ssl' => 'SSL Security',
            'ssl_sub' => '256-bit encryption',
            'cancel_anytime' => 'Cancel Anytime',
            'cancel_sub' => 'No commitments',
            'secure_payment' => 'Secure Payment',
            'secure_sub' => 'VNPay, MoMo, Cards',
            'support_247' => '24/7 Support',
            'support_sub' => 'Pro & Enterprise plans',
        ],

        // FAQ
        'faq' => [
            'title' => 'FAQ',
            'heading' => 'Frequently Asked Questions',
            'items' => [
                [
                    'q' => 'Can I try before purchasing?',
                    'a' => 'Yes! Pro plan includes a 7-day free trial, no credit card required. You can experience all features and decide later.',
                ],
                [
                    'q' => 'What are the benefits of annual payment?',
                    'a' => 'Annual payment saves you 20% compared to monthly. With Pro plan, you save 480,000đ per year.',
                ],
                [
                    'q' => 'Can I upgrade or downgrade anytime?',
                    'a' => 'Yes. You can upgrade immediately. Downgrading will take effect at the next billing cycle.',
                ],
                [
                    'q' => 'Does Mindigo support VAT invoices?',
                    'a' => 'Yes. After payment, you can request a VAT e-invoice via support@mindigo.vn within 5 working days.',
                ],
                [
                    'q' => 'Who is the Enterprise plan suitable for?',
                    'a' => 'Enterprise plan is suitable for universities, training centers, and businesses with 50+ users that need SSO, custom API, or white-label solutions.',
                ],
            ],
        ],
    ],
];
